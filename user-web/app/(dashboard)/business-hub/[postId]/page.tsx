...
/**
 * Business Hub Post Detail Page
 * Full view of a business hub post with content and interactions
 */
...
export default function BusinessHubPostDetailPage({
  params,
}: PostDetailPageProps) {
  const { postId } = use(params);
  const [post, setPost] = useState<BusinessHubPost | null>(null);
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data, error: fetchError } =
        await getBusinessHubPostById(postId);
      if (fetchError) {
        setError(fetchError);
      } else {
        setPost(data);
  ...
  if (error || !post) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] text-center px-4">
        ...
        <Button asChild>
          <Link href="/business-hub">Back to Business Hub</Link>
        </Button>
      </div>
    );
  }
    ...
    >
      ...
      <article className="bg-white dark:bg-slate-900 rounded-2xl border border-border/50 overflow-hidden shadow-sm">
        ...
        <div className="p-6 space-y-5">
          {/* Category, Company, Industry, and Tags */}
