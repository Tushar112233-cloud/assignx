...
import { Supervisor, SupervisorActivation, User, Project, Doer, UserDoerReview, SupervisorDoerReview, SupervisorWallet, WalletTransaction, SupportTicket, Subject, Notification, TrainingModule, ChatRoom, QuizQuestion } from '../models';
...
router.get('/reviews', authenticate, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const filter: any = { supervisorId: req.user!.id };
    ...
    const reviews = await SupervisorDoerReview.find(filter)
      .sort({ createdAt: -1 })
      .skip(Number(offset))
      .limit(Number(limit))
      .populate('doerId', 'name email')
      .populate('projectId', 'title');
    const total = await SupervisorDoerReview.countDocuments(filter);
...
router.get('/activation/quiz/:quizId', authenticate, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const questions = await QuizQuestion.find({ moduleId: req.params.quizId, isActive: true })
