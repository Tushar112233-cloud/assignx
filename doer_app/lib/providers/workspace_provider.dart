...
class WorkspaceNotifier extends ChangeNotifier {
  ...
  Future<bool> sendMessage(String content) async {
    ...
    try {
      ...
      if (message != null) {
        // Guard against duplicate: the socket listener in
        // _subscribeToChatUpdates may have already added this message
        // before the HTTP response arrived.
  ...
  Future<bool> sendFileMessage({
    ...
    try {
      ...
      if (message != null) {
        // Guard against duplicate: the socket listener may have already
        // added this message before the HTTP response arrived.
        final alreadyExists = _state.messages.any((m) => m.id == message.id);
        if (!alreadyExists) {
          _updateState(_state.copyWith(
            messages: [..._state.messages, message],
