...
class ActiveChatNotifier extends StateNotifier<ActiveChatState> {
  ...
  Future<bool> approveMessage(String messageId) async {
    try {
      final success = await _repository.approveMessage(messageId);
      if (success) {
        state = state.copyWith(
          messages: state.messages
              .map((m) => m.id == messageId
                  ? m.copyWith(approvalStatus: 'approved')
                  : m)
              .toList(),
        );
      }
  ...
  Future<bool> rejectMessage(String messageId) async {
    try {
      final success = await _repository.rejectMessage(messageId);
      if (success) {
        state = state.copyWith(
          messages: state.messages
              .map((m) => m.id == messageId
                  ? m.copyWith(approvalStatus: 'rejected')
                  : m)
              .toList(),
        );
      }
