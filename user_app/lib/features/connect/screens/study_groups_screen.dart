...
class _StudyGroupsScreenState extends ConsumerState<StudyGroupsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final subjects = ref.watch(connectSubjectsProvider).valueOrNull ?? [];
  ...
  Widget _buildEmptyState() {
    // When a subject filter is active, show a "no results" state.
    // Otherwise this is the default stub state -- show "Coming Soon".
    if (_selectedSubject != null) {
      return GlassCard(
        blur: 12,
        opacity: 0.8,
    ...
    return GlassCard(
      ...
      child: Column(
        children: [
          ...
          const SizedBox(height: 12),
          Text(
            'Study Groups Coming Soon'.tr(context),
            style: AppTextStyles.headingSmall.copyWith(
              color: AppColors.textPrimary,
            ),
          ),
          const SizedBox(height: 8),
          Text(
            'Collaborative study groups are on the way!'.tr(context),
            style: AppTextStyles.bodyMedium.copyWith(
              color: AppColors.textTertiary,
            ),
            textAlign: TextAlign.center,
