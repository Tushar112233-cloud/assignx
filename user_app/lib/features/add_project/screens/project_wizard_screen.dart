...
class _ProjectWizardScreenState extends ConsumerState<ProjectWizardScreen> {
  ...
  DateTime? _deadline;
  PricingConfig _pricing = const PricingConfig();
  ...
  Future<void> _loadPricing() async {
    final config = await PricingService.fetch();
    if (mounted) setState(() => _pricing = config);
  }

  /// Calculate base price based on project type using admin-configured pricing.
  double? _calculateBasePrice() {
    switch (_projectType) {
      case ProjectType.assignment:
      case ProjectType.document:
        if (_wordCount == null) return null;
        return _wordCount! * _pricing.assignmentPerWord;
      case ProjectType.website:
        if (_pageCount == null) return null;
        return (_pageCount! * _pricing.websitePerPage) +
            (_websiteFeatures.length * _pricing.websitePerFeature);
      case ProjectType.app:
        return _platform == 'both'
            ? _pricing.appBothPlatforms
            : _pricing.appSinglePlatform;
      case ProjectType.consultancy:
        if (_consultancyDuration == '30min') return _pricing.consultancy30min;
        if (_consultancyDuration == '2hr') return _pricing.consultancy2hr;
        return _pricing.consultancy1hr;
      case ProjectType.turnitinCheck:
        return _pricing.turnitinCompleteReport;
      case null:
        return null;
    }
  }
